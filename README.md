...
| **Drivetrain** | **Built and driven (final chassis — rebuild cancelled 2026-08-10)** — N20 via TB6612 through a LEGO differential; Ackermann steering. Working point (pinion:crown ratio, currents) still unmeasured. |
...
|---|---|---|---|
| 1 | Mobility & Mechanical Design | [`docs/1_mobility.md`](docs/1_mobility.md) | Steering + drive integrated, driven on the mat, retune 2026-08-10 documented; **chassis final (rebuild cancelled 2026-08-10)**; pinion:crown ratio still uncounted |
...
models/         CAD / printable parts        — empty; scope = printed mount STLs (rebuild cancelled 2026-08-10, no chassis CAD)
schemes/        electromechanical schematics — signal + power wiring v0.2 (generated)
t-photos/       team photos                  — pending
v-photos/       vehicle photos               — 6 views present (final chassis)
video/          performance video links      — pending a recorded complete run (vehicle drives; see other/)
...
|---|---|---|
| 6 vehicle photos — every side, top and bottom | Yes | **Present** — `v-photos/`, final chassis (rebuild cancelled 2026-08-10); refreshed once the Pi 5 + camera are mounted |
| 2 team photos | Yes | **Pending** |
| Performance video, >= 30 s autonomous driving, one per challenge | Yes | **Pending** — the vehicle drives (test laps in `other/`); the slot needs a recorded complete run through the autonomous stop, per [`video/video.md`](video/video.md) |
...
| CAD / printable parts | Yes | **Pending** — printed mount STLs (3x Luna, servo, N20) uncollected; no chassis CAD will exist (rebuild cancelled 2026-08-10) |
