...
- **Not yet logged: a full run of this stack.** A run started as `python main.py 2>&1 | tee run_N.log` records every transition with its reason, which gives corners completed, pillars passed, recoveries and faults per run. For parking, `parallel_parking.py` with `ENABLE_RUN_LOGGING = True` (committed as `False`) writes `run_logs/parking_run_<time>.csv`, one row per frame (state, heading, the three distances, the command, both blocks' floor positions and the slot estimate), and an annotated video; `partial_parking.py` prints its state and slot estimate every 0.5 s. Either gives the attempts and how each ended, COMPLETE or FAULT with its reason; the CSV also gives the time taken.
## 8. Parking modules
