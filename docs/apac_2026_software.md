...
- **Run logs exist on the car, not here yet.** The 2026-09-18 `main.py` writes `run_logs/round2_run_<date_time>.csv` beside itself on every run (`ENABLE_RUN_CSV_LOGGING = True`): one row per telemetry sample and one per event, with lap, corner count, pillars this straight, state, heading and target, the three distances, the command sent and applied, the tracked pillar with its band, and the tape depths. Those files give corners completed, pillars passed, recoveries and faults per run; none is in this repository yet. For parking, `parallel_parking.py` with `ENABLE_RUN_LOGGING = True` (committed as `False`) writes a CSV and an annotated video per attempt (§8).
## 8. Parking modules

Two files beside `main.py`, added on 2026-09-23, park the car at the end of the run. `main.py` does not call them: v0.4.0 had parking at the end off, and the 2026-09-18 version parks with its own routine (§10): each runs on its own and imports `main.py` for the camera, the serial link, telemetry, the live view and the shutdown. Constants named in this section are in the parking files.
...
**At APAC the team parks with `parallel_parking.py`** (decided 2026-09-23). A full parallel park scores 15 points against 7 for a partial or non-parallel one (2026 General Rules, scoring element 1.8.2), and touching a limitation ends the round (rule 9.24.7): the entry is deliberately slow (speed 40, a "slow, controlled approach" in the code) and the reverse strokes are capped so the car cannot back out. The `main.py` that hands over to it after the twelfth corner is not yet in this repository; `partial_parking.py` is kept as the fallback. The `main.py` now in the repository (2026-09-18) parks with its own routine instead (§10). Every `main.py` name the parking modules use also exists in the 2026-09-18 file.
...
What `UnifiedPillarDetector` does with each 1280 × 720 frame, and why. Constants are in `main.py` (the 2026-09-18 file; v0.4.0 values in brackets where they differ); the two figures at the end show the result on the mat.
...
2. **Central view.** Before detection a strip on each side of the frame is set to neutral grey (128, outside every Lab range) without moving pixel coordinates: 16 % of the width on each side (`CAMERA_SIDE_MASK_FRACTION`), 25 % for 5 s after a corner (`POST_CORNER_VIEW_SIDE_FRACTION`, `POST_CORNER_NARROW_VIEW_SECONDS`), none after the twelfth corner, so parking sees the whole frame. Not in v0.4.0.
...
8. **Two pillars per straight** (2026-09-18 file). At most two pillars are taken on each straight (`MAX_PILLARS_PER_STRAIGHT = 2`); once both are passed, no new pillar is taken until the corner (live view: CORNER NEXT - NO NEW PILLAR). One high track in the next lane is deferred rather than its whole colour suppressed, and right after a turn nearby pillars take priority (`defer_background_candidate`). A wide, low rectangle, at least twice as wide as tall, is treated as a parking-lot limitation, not a pillar; in laps 1 and 2 the car steers gently around it (speed 60, steering 15).
...
## 10. What the 2026-09-18 main.py changes

The file now in the repository has 6,663 lines against 4,771 in v0.4.0: about 2,430 lines added and 540 removed. This section lists the changes from the code; where the code gives a reason in a comment or docstring, it is quoted or paraphrased. The team removed the side rollers after this work ([vehicle §7](apac_2026_vehicle.md)).

**Walls.** Three additions keep the car in the middle of its lane:
...
- After a corner the same kind of side-distance correction recentres the car (`ROUND2_RECENTER_*`: gain 0.45, at most 14, heading within 8°).
- CORNER_CLEARANCE_RECOVERY reverses until the front has 30 cm of clearance and re-checks the corner while stopped; it ends in FAULT if both side walls are too close for a corner reverse, or if the ESP32 does not report the reverse as applied. `TURN_SIDE_CLEARANCE_CM` is 20 (501 in v0.4.0, which carried 20 as a comment).

**Corners.** The corner approach is entered after three fresh front readings of 105 cm or less ("Enter the real corner approach after three fresh geometry samples", `CORNER_ZONE_LOCK_*`); after a corner the recognisers stay disarmed until the centre sensor sees a clearly open straight (130 cm, 3 samples), while the camera keeps detecting pillars. New states: PRE_TURN_BACKUP (0.28 s back at speed 88 before a turn) and POST_TURN_SCAN_HOLD, a 0.2 s stop after the post-turn reverse "so several stationary frames can prove a nearby pillar". The post-turn reverse is 0.64 s at speed 125 (2.8 s in v0.4.0).

**Pillars.** Two per straight, the central view and the parking-lot rule (§9). The red target column moved from 25 % to 38 % of the width, the near band starts at 3,200 px×px (4,000), the pass speed is 80 (100) and the release speed 80 (50).

**Parking at the end.** `ENABLE_PARKING_IN = True`. After the twelfth corner: PARKING_ENTRY_SEARCH (the final straight, pillars still handled), PARKING_BLOCK_SEARCH (the camera looks for the limitation), PARKING_IN_ENTER (camera-guided entry), PARKING_IN_STOP_HOLD (0.2 s), PARKING_IN_REVERSE_LEFT (3.0 s at full left lock), PARKING_IN_REVERSE_RIGHT (0.2 s), then COMPLETE. v0.4.0's PARKING_IN_ANTI_* and PARKING_IN_CLOCK_ARC states are gone. The team plans to park with `parallel_parking.py` at APAC instead (§8).

**Run log.** `RunCsvLogger` writes every run to `run_logs/round2_run_<date_time>.csv` (§7).
...
| 1, 5, 6 | COMPLETE after the twelfth corner; parking at the end off | parking-in follows the twelfth corner |
...
| 4 | post-turn reverse 2.8 s | 0.64 s at speed 125, after a 0.28 s reverse before the turn |
