...
| R10 | ~~No drive motor selected; propulsion is unbuilt~~ **Superseded 2026-08-06:** propulsion is built — N20 through a TB6612 into a spur pinion and rear-axle differential, integrated in `406e315`. The live risk is now **wheel retention**: the LEGO wheels shed within ~5 s of sustained drive. | Certain | **Critical** | Chassis rebuild to a 3D-printed frame is in fabrication; until the new frame passes the sustained-drive test that killed the old one, no run longer than a few seconds is trustworthy. | **Open — rebuild in fabrication** |
