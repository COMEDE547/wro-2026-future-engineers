...
**Status 2026-09-20.** After the WRO India National Championship (26–28 Aug 2026) the vehicle was rebuilt for APAC 2026. This page records what changed and why. Every figure is marked by its basis: *measured* on the vehicle, *team* figure not yet measured, *derived* by calculation from named inputs, or *vendor* specification. Documents 1–5 remain the record of the Nationals configuration.
...
- **Voltage the motor sees:** the TB6612FNG switches the pack (11.1 V nominal, 12.6 V full) with PWM, so the motor's average voltage is duty × pack voltage. With the Nationals firmware, full commanded speed was 100 % duty (12.47 V measured at the motor terminals) and half speed was 48 % duty (5.95 V) — see [2a](2_power_predicted_budget.md). On a 6 V motor, duty above about 48 % on a full pack averages more than its rating. The APAC firmware's duty limit goes here when the code is committed; a motor-temperature check after a full 3-minute run is planned.
...
- **LEDs:** 6 × WS2812 on the front frame; data from ESP32 GPIO4; power from the 5 V Buck-2 rail (team). Planning current: up to 60 mA per LED at full white (the [Adafruit NeoPixel Überguide](https://learn.adafruit.com/adafruit-neopixel-uberguide) rule of thumb), so at most 0.36 A added to the 5 V rail (derived, worst case; the colour and brightness actually used are set in the firmware).
...
## 6. New failure points (open)

| Risk | Effect | Check or mitigation |
|---|---|---|
| Pi 5 fed from a USB-A fast-charge module (no USB-PD) | The Pi limits its USB ports to 600 mA in total; the camera and the ESP32 both draw from them | Run `vcgencmd get_throttled` after a full run; move to a 5 A USB-C PD supply if it reports problems |
...
| 6 V motor on the 11.1 V pack | Heat and brush wear if the duty limit is above about 48 % | Duty limit from the firmware; temperature check after a 3-minute run |
...
From the Nationals firmware ([`round2_ino.ino`](../src/Round%202/round2_ino/round2_ino.ino)) and the 2026-09-20 wiring draft; only GPIO4 is new. To be re-checked against the APAC firmware when it is committed.
...
- The APAC firmware and Raspberry Pi code, which settle the duty limit (§3) and the pin re-check (§8).
- Measurements: turning circle at walking pace and race speed, the blind-zone tape check, motor speed and current, current on each supply line.
