...
#         # print("measurement matrix size:", kf.measurementMatrix.shape)
...
#         smoothed = {'center_x': int(prediction[0]), 'center_y': int(prediction[1]), 'vx': float(prediction[2]), 'vy': float(prediction[3]),}
#         return smoothed, initialized
#     else:
#         prediction = kf.predict()
#         # smoothed = {'center_x': predicted[0, 0], 'center_y': predicted[1, 0]}
#         smoothed = {'center_x': int(prediction[0]), 'center_y': int(prediction[1]), 'vx': float(prediction[2]), 'vy': float(prediction[3]),}
#         return smoothed, initialized
...
def main(camera_id: int = 0, frame_size: int = 240):
    ...
    try:
        while True:
            ...
            active_box = red_box if current_detection == 'red' else (
                         green_box if current_detection == 'green' else None)
            ...
            cv2.line(display, (LEFT_SIDE_MAX, 0), (LEFT_SIDE_MAX, frame_size-1), (0,255,255), 1)
            cv2.line(display, (RIGHT_SIDE_MIN, 0), (RIGHT_SIDE_MIN, frame_size-1), (0,255,255), 1)
            display = upscale_for_display(display, scale=3)
            cv2.imshow(window_name, display)
